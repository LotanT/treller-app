...
function _AddAttachment(props) {
  ...
  const uploadFile = async (ev) => {
    try {
      ...
      currTask.attachments = currTask.attachments
        ? [...currTask.attachments, attach]
        : [attach];
  ...
  const onAttachmentClick = async () => {
    if (attachSrc) {
      const attach = {
        ...
        url: attachSrc.startsWith("http" || "https")
          ? attachSrc
          : "https://" + attachSrc,
        createdAt: Date.now(),
        isWeb: true,
      ...
      <div className="pop-content">
        ...
        <div className="add-label-preview">
          ...
          <input
            ...
            value={attachSrc}
            name="webUrlSrc"
            onChange={(e) => setAttachSrc(e.target.value)}
          />
          {attachSrc && (
            <>
          ...
          <button className="attach-btn" onClick={onAttachmentClick}>
            Attach
          </button>
        </div>
      </div>
