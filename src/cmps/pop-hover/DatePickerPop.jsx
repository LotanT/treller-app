...
function _DatePickerPop(props) {
    ...
    const onRemoveDueDate = async () => {
        let updatedBoard = taskService.removeDueDateToTask(props.board, props.taskId)
        await props.onEditBoard(updatedBoard)
        props.toggleModal()
    }
        ...
        <div className="add-labels-pop" style={{top: top,left: left}}>
            ...
            <GrClose stroke="#0079bf" fill="#0079bf" className='exit-svg' onClick={props.toggleModal} />
                ...
                <div className="add-label-preview">
                    <button className='btn btn-due-date-create' onClick={onSaveDueDate} >Save</button>
                    <button className='btn btn-due-date-remove' onClick={onRemoveDueDate} >Remove</button>
                </div>
            </div>
        </div>
    )
...
export const DatePickerPop = connect(mapStateToProps, mapDispatchToProps)(_DatePickerPop)