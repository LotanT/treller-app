...
function _TaskAttachmentsPreview(props) {
    ...
    const [attachImg, setAttachImg] = useState(attach.url ? attach.url : "https://i.ibb.co/cJq4Hbk/file.jpg")
    ...
    return (
        <div className="attach-thumb">
            ...
            <div className="attach-details">
                <span className="attach-details-title">{attach.name.substring(0, 45)}</span><br />
                <span className='attach-quit-details'>
                    <span>Added {utilService.timeSince(attach.createdAt)}</span>
                    {isAttachImg && <div className='flex make-cover-attach' onClick={toggleCover}><CgScreenWide /> <a>{makeItCoverTxt}</a></div>}
                </span>
            </div>
        </div>
    )
...
export const TaskAttachmentsPreview = connect(mapStateToProps, mapDispatchToProps)(_TaskAttachmentsPreview)